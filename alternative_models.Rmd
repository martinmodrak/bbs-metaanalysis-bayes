...
Using the simplest model, we see that cLOF differences are not well modelled for the CI, REP, REN and LIV phenotypes. 
...
Adding source as a covariate ameliorates the problems with cLOF (it remains problematic only for REN and LIV phenotypes):
...
We see the `gene_source_lof` model already does not do badly in predicting prevalence within families, although including family in the model is definitely an improvement for some families.
