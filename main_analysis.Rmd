...
- 
    data uvedena v pořadí RD - OBE - PD - CI - REP - REN - HEART - LIV
    colors: BBS3 "red" #f8766dff, BBSome "blue" #5f8dd3ff, chaperonins "gold" #aa8800ff
    aby na osách vždycky bylo měřítko, co je skutečně měřítko :-)
    kdyby popisky mohly být Arial vel. 7, bylo by to úplně nejlepší
...
```{r}
formula_gene_source_lof
```

TODO LOF do math notation
...
#The file argument
fit_gene_source_lof <- brm(formula_gene_source_lof, prior = priors, data = data_long, control = list(adapt_delta = 0.9),
                     file = here(stored_fits_dir,"gene_source_lof"))
fit_gene_source_lof
...
data_for_prediction_gene_source_lof_BBSome <- data_for_prediction_gene_source_lof %>% filter(functional_group_for_gene(gene) == "BBSome", gene != "BBS18")
```
...
```{r, fig.width=8, fig.height=4}
differences_func_group_plot <- plot_gene_phenotype_differences_estimates(fit_gene_source_lof, data_for_prediction_gene_source_lof %>% mutate(group = functional_group), genes_to_show = genes_to_show, group_title = "Functional group", data_original = data_long %>% mutate(group = functional_group))
...
```{r, fig.width=8, fig.height=4}
differences_bbsome_plot <- plot_gene_phenotype_differences_estimates(fit_gene_source_lof, data_for_prediction_gene_source_lof_BBSome, genes_to_show = genes_to_show, data_original = data_long)
...
data_for_prediction_gene_source_lof_chaperonins <- data_for_prediction_gene_source_lof %>% filter(functional_group_for_gene(gene) == "Chaperonins")


differences_chaperonins_plot <- plot_gene_phenotype_differences_estimates(fit_gene_source_lof, data_for_prediction_gene_source_lof_chaperonins, data_original = data_long)
...
```{r}
lof_plot <-  plot_lof_differences_estimates(fit_gene_source_lof, data_for_prediction_gene_source_lof_BBSome)
lof_plot
```
...
```{r, fig.width=8, fig.height=4}
plot_pairwise_differences(fit_gene_source_lof, data_for_prediction_gene_source_lof_BBSome,  "heatmap_min")
```
...
plot_pairwise_differences(fit_gene_source_lof, data_for_prediction_gene_source_lof_BBSome, plot_types = "heatmap_max")
```
...
plot_pairwise_differences(fit_gene_source_lof, data_for_prediction_gene_source_lof_BBSome, plot_types = "linerange_all")

```


```{r, fig.width=8, fig.height=4}
plot_lof_differences_estimates(fit_gene_source_lof, data_for_prediction_gene_source_lof_BBSome)
```
...
for(type in types) {
  ...
  lof_plot %>% ggsave(paste0("loss_of_function", type), plot = ., path = img_path,height = std_height, width = std_width)
  
  lof_data_plot %>% ggsave(paste0("loss_of_function_data", type), plot = ., path = img_path,height = std_height, width = std_width)
  
  plot_pairwise_differences(fit_gene_source_lof, data_for_prediction_gene_source_lof_BBSome, plot_types = c("heatmap_min", "heatmap_max"), out_func = function(name, plot) {
    ggsave(paste0(name, type), plot = plot, path = img_path,height = std_height, width = std_width)
  })
  
  plot_pairwise_differences(fit_gene_source_lof, data_for_prediction_gene_source_lof_BBSome, plot_types = "linerange_all", out_func = function(name, plot) {
...
plot_pairwise_differences(fit_gene_source_lof, data_for_prediction_LIV, plot_types = "linerange_all")
