...
This includes the Ullah et al. study which has the largest number of BBS07 cases with reported CI phenotype in the whole dataset. So while there is some evidence for BBS07 having unusually high prevalence of CI, we cannot rule out that this is caused by between-study variability.

The above plot looked at total prevalence across all other mutations, we can also look at comparisons of prevalence between individual mutations:
...
Lets look at comparison of BBS02 to prevalence across all other phenotypes:

```{r, fig.width = 6, fig.height = 2.5}
plot_diff_average_to_single_gene(data_long, "HEART", "BBS02", bbs_genes_to_check)
```

We see there is one study showing higher prevalence, one study showing lower prevalence and two showing equal prevalence. All the other studies either had only BBS02 or had no BBS02 and so cannot directly contribute to a conclusion, despite showing small prevalence of HEART. Once again, we cannot rule out the difference between BBS2 and others in HEART phenotype is simply due to between-study variability.

### LIV BBS2 to BBS1 comparison

In the data BBS02 has higher LIV prevalence than BBS01 (again statistically significant, despite the small sample sizes):
...
```
There is one study that show increase for BBS02 and one that shows decrease for BBS02. And the one showing decrease is actually the studies with larger sample size. The difference seems to be driven by studies that have BBS01 patients no or almost no BBS02 patients and can thus be attributed to between-study variability.
