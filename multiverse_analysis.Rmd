...
*This is Appendix 2 for the paper "Exploring the function of the BBSome using clinical data: Meta-analysis of genotype-phenotype associations in Bardet-Biedl Syndrome" The complete source code for the analysis can be found at https://github.com/martinmodrak/bbs-metaanalysis-bayes *
...
This appendix describes all Bayesian models we tried throughout this project. Further we discuss the reasoning behind our choice of model for the main analysis, in particular why between-study variability is crucial and cLOF useful while age, and sex can be omitted. Finally, we compute how the conclusions of the paper hold under all of the models.
...
Modelling between-study variability is simply a good practice for any metaanalysis.
...
We see that the model is overly certain in its predictions and the credible intervals frequently miss the actual counts in a study. 11 / 25 sources have predictions not very consistent with data.

This does not resolve when including cLOF or age/sex (here using the filtered model, as we don't really care)

```{r}
pp_check_helper("gene_filtered_age_sex", "source_10")
```
6/14 off in gene_filtered_age_sex

```{r}
pp_check_helper("gene_lof_per_gene", "source_10")
```

10/25 off gene_lof_per_gene
...
6/25 problematic in gene_source

```{r}
pp_check_helper("gene_source_lof", "source_10")
```

Resolves when including cLOF as well.
