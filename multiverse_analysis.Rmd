...
freq_conclusions <- freq_conclusions %>% 
  ...
  mutate(model_nice = paste0("frequentist ", gsub("p-value-","", model_name)), Category = "Main")
```
...
freq_conclusions_plot <-  freq_conclusions %>% ggplot(aes(x = model_nice, y = Conclusion, fill = p)) + geom_tile() + 
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust =0.5)) + 
    #scale_fill_gradient(low = "#7f0000", high = "#f0f0f0") +
    # scale_fill_gradientn(colours = c("red", "blue"),
    #                     values = log10(c(min(freq_conclusions$p, na.rm = TRUE), 1))) +
...
#                         values = log10(c(min(freq_conclusions$p, na.rm = TRUE), 0.049, 0.05, 1))) +
...
combined_conclusions_plot <- plot_grid(conclusions_plot + gg, freq_conclusions_plot + theme(axis.title.y = element_blank(), axis.text.y = element_blank()) + gg, align = "h",rel_widths = c(5, 1))
