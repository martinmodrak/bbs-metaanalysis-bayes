...
```{r}
evaluators[["functional_groups_differ"]] <- function(samples_diff, samples_diff_bbsome, samples_prediction_bbsome, samples_linear_bbsome) {
  samples_diff %>% 
    ...
    ungroup() %>%
    summarise(`Functional groups differ` = mean(enough_phenotype_relevant))
}
```
...
```{r}
evaluators[["bbs04_bbs05_cor"]] <- function(samples_diff, samples_diff_bbsome, samples_prediction_bbsome, samples_linear_bbsome) {
  samples_linear_bbsome %>% 
    filter(gene %in% c("BBS04","BBS05")) %>%
    spread(gene, value) %>%
    group_by(sample) %>%
    summarise(correlation = cor(BBS04,BBS05)) %>%
    ungroup() %>%
    summarise(`BBS4 and BBS5 correlate` = mean(correlation > 0.8))
}
```
...
```{r}
evaluators[["bbs04_highest"]] <- function(samples_diff, samples_diff_bbsome, samples_prediction_bbsome, samples_linear_bbsome) {
samples_diff_bbsome %>% 
  filter(gene.x == "BBS04", gene.y != "BBS04") %>%
  group_by(sample) %>%
  summarise(bbs04_highest = all(odds_ratio >= 1 / min_interesting_effect)) %>%
  summarise(`BBS4 highest odds` = mean(bbs04_highest))
}
```
```{r}
evaluators[["p_most_pheno"]] <- function(samples_diff, samples_diff_bbsome, samples_prediction_bbsome, samples_linear_bbsome) {
...
```{r}
evaluator_prob_bbsome_diff_irrelevant <- function(phenotype_name) { 
...
evaluators[["OBE_no_diff"]] <- evaluator_prob_bbsome_diff_irrelevant("OBE")
```  
*	There are no apparent differences in the penetrance of obesity between functional groups of BBS genes.
  * Očekávám, že nebude potvrzeno.
  * Stejně jako výše.
```{r}
evaluators[["RD_no_diff"]] <- evaluator_prob_bbsome_diff_irrelevant("RD")
```
...
evaluator_prob_group_diff_random <- function(phenotype_name, group1, group2, group1_name = paste0(group1, collapse = "_"),group2_name = paste0(group2, collapse = "_")) {
      function (samples_diff, samples_diff_bbsome, samples_prediction_bbsome, samples_linear_bbsome) {
...
evaluators[["CI_BBSO3_random"]] <- evaluator_prob_group_diff_random("CI", paste0("BBS0", c(1,2,4,5,7,8,9)), "BBS03", group1_name = "BBSome")
```
...
evaluators[["CI_BBS07_random"]] <-  evaluator_prob_group_diff_random("CI", "BBS07", paste0("BBS0", c(1,2,4,5,8,9)), group2_name = "others")
...
evaluators[["REN_groups_random"]] <- evaluator_prob_group_diff_random("REN", paste0("BBS0", c(2,7,9)), paste0("BBS0", c(1,4,8)))
```
...
evaluators[["HEART_BBS2_random"]] <-  evaluator_prob_group_diff_random("HEART", "BBS02", paste0("BBS0", c(1,4,5,7,8,9)), group2_name = "others")
...
evaluators[["HEART_BBS5_random"]] <-  evaluator_prob_group_diff_random("HEART", "BBS05", paste0("BBS0", c(1,2,4,7,8,9)), group2_name = "others")
```
...
evaluators[["HEART_BBS4_random"]] <-  evaluator_prob_group_diff_random("HEART", paste0("BBS0", c(1,2,5,7,8,9)), "BBS04", group1_name = "others")
```
...
conclusions_plot <- evaluation %>% 
  ...
  mutate(Conclusion = factor(Conclusion, levels = sort(unique(Conclusion), decreasing = TRUE)),
         Category = if_else(model_name %in% c("gene_source","gene_source_filtered_lof"), "Main", "Secondary")) %>%
  ggplot(aes(x = model_name, y = Conclusion, fill = Probability)) + geom_raster() + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust =0.5)) + scale_fill_distiller("Posterior prob.", type = "div", palette = 3, limits = c(0,1)) + facet_grid(.~Category, scales = "free_x", space = "free_x")
