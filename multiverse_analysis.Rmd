...
evaluator_lof <- function(def, fit) {
 if("lof" %in% def$additional_components || "lof per gene" %in% def$additional_components)  {
   ...
   samples_tidy %>% 
     group_by(sample, phenotype) %>% summarise(enough_genes = sum(odds_ratio > 1) >= n_genes_different) %>%
     group_by(sample) %>% summarise(enough_phenotypes = sum(enough_genes) >= min_phenotypes_for_diffs) %>%
     ungroup() %>% summarise(`LOF mutations more severe` = mean(enough_phenotypes))
 } else {
   tibble(`LOF mutations more severe` = NA_real_)
 }
}
```
...
evaluator_prob_pairwise_diff <- function(phenotype_name, gene_more_frequent, gene_less_frequent) {
  function (samples_diff, samples_diff_bbsome, samples_prediction_bbsome, samples_linear_bbsome) {
     target_column_name = paste0(phenotype_name,": ", bbs_labeller(gene_more_frequent), " more likely than ", bbs_labeller(gene_less_frequent))
      samples_diff %>% 
...
evaluator_prob_group_diff_all <- function(phenotype_name, group1, group2, group1_name = paste0(group1, collapse = "_"),group2_name = paste0(group2, collapse = "_")) {
    function (samples_diff, samples_diff_bbsome, samples_prediction_bbsome, samples_linear_bbsome) {
      ...
      target_column_name = paste0(phenotype_name,": ",bbs_labeller(group1_name)," more likely than ", bbs_labeller(group2_name))
      samples_diff %>% 
...
nice_model_name <- function(x)  {
  x %>% gsub("only","none", .) %>% gsub("gene_","",.) %>% gsub("filtered_","filtered ", .) %>%
    ...
    gsub("lof_per_gene","lof (by gene)", .) %>% 
    gsub("cor"," corr.", .) %>% 
    gsub("imputed_","imputed ", .) %>%  gsub("_"," + ", .)
}

conclusion_names <- rev(names(evaluation)[-length(names(evaluation))])
...
evaluation_processed <- evaluation %>% 
  ...
  mutate(Conclusion = factor(Conclusion, levels = conclusion_names),
...
conclusions_plot <- evaluation_processed %>%
  ggplot(aes(x = model_nice, y = Conclusion, fill = Probability)) + geom_tile() + 
    ...
    #scale_fill_gradientn("Posterior prob.", colours = c("#1b7837", "#d9f0d3","#e7d4e8", "#762a83"),values = c(0, 0.25,0.75,1),  limits = c(0,1)) +
    #scale_fill_gradientn("Posterior prob.", colours = c("#1b7837", "#a6dba0","#c2a5cf", "#762a83"),values = c(0, 0.25,0.75,1),  limits = c(0,1)) +
    scale_x_discrete("Model components/modifications besides gene") +
    facet_grid(.~Category, scales = "free_x", space = "free_x") + base_theme
...
if(!identical(sort(freq_conclusions$Conclusion), sort(conclusion_names))) {
  setdiff(freq_conclusions$Conclusion, conclusion_names)
  setdiff(conclusion_names, freq_conclusions$Conclusion)
  stop("Conclusions mismatch")
}

freq_conclusions <- freq_conclusions %>% 
  mutate(Conclusion = factor(Conclusion, levels = conclusion_names), model_nice = "frequentist all", Category = "Main")
```

```{r}
freq_conclusions_plot <-  freq_conclusions %>% ggplot(aes(x = model_nice, y = Conclusion, fill = `p-value`)) + geom_tile() + 
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust =0.5)) + 
    scale_fill_gradientn(colours = c("#7f0000", "#fdd49e","#f0f0f0", "#f0f0f0"),
                         values = c(min(freq_conclusions$`p-value`), 0.049, 0.05, 1), trans = "log10") +
...
```{r}
conclusions_plot %>% ggsave(here("tmp_pics","conclusions.svg"), ., width = 6, height = 4, dpi = 500)
```
