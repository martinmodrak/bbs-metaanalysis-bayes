...
```{r}
priors <- c(prior(normal(0,2), class = Intercept), prior(normal(0,1), class = sd))
...
```{r}
priors <- c(prior(normal(0,2), class = Intercept), prior(normal(0,0.1), class = sd))
...
formula_gene_source_sex <- brmsformula(update.formula(formula_gene_source,  ~ . + (1||Sex + Sex : phenotype)) , family = "bernoulli")

priors <- c(intercept_prior, b_prior, sd_prior)
