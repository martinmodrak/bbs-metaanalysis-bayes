...
data <- read_excel(here("private_data","--META-DATA-5.xlsx"), sheet = "DATA 5") %>%
  ...
  mutate(Age_corr = if_else(Age == "5 month", as.character(5/12), gsub(",",".", Age)),
         age_group = factor(case_when(
           ...
           !is.na(as.numeric(Age_corr)) ~ 
             case_when(
               as.numeric(Age_corr) < 10 ~ "0-9",
               as.numeric(Age_corr) < 20 ~ "10-19",
               as.numeric(Age_corr) < 30 ~ "20-29",
               as.numeric(Age_corr) < 40 ~ "30-39",
               as.numeric(Age_corr) < 50 ~ "40-49",
               as.numeric(Age_corr) < 60 ~ "50-59",
               TRUE ~ "60+",
             ),
           TRUE ~ Age_corr
           ))
...
genes_to_use <- sprintf("BBS%02d",1:12)
